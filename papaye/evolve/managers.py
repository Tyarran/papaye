...
class PapayeEvolutionManager(object):
    ...
    def step(self, version, evmodule):
        current_version_str = colored(
            str(version - 1), 'yellow', attrs=('bold', )
        )
        version_str = colored(
            str(version), 'yellow', attrs=('bold', )
        )
        script_str = colored(evmodule.__name__, 'green', attrs=('bold', ))
        print('\n>>> Evolving ({} => {}) {}'.format(
            current_version_str, version_str, script_str)
        )

    def information(self, title, string):
        print('\t{} {}:\t {}'.format(
            colored('*', 'green', attrs=('bold',)), title, string)
        )

    def evolve_to_current(self):
        if self.last_evolve_model:
            last_evolve_model_str = colored(
                str(self.last_evolve_model.__name__),
                'yellow',
                attrs=('bold', )
            )
            current_model_str = colored(
                'papaye.models', 'yellow', attrs=('bold', )
            )
            self.task('Evolving {} to current model {}'.format(
                last_evolve_model_str, current_model_str)
            )
            module_name = self.last_evolve_model.__name__
